...
special_effects = dict(
    long_paddle=(
        colors.ORANGE,
        lambda g: g.paddle.bounds.inflate_ip(config.paddle_width // 2, 0),
        lambda g: g.paddle.bounds.inflate_ip(-config.paddle_width // 2, 0)
    ),
    slow_ball=(
        colors.AQUAMARINE2,
...
assert os.path.isfile(os.path.join('sound_effects', 'brick_hit.wav'))
...
class Breakout(Game):
    ...
    def handle_ball_collisions(self):
        ...
        for brick in self.bricks:
            ...
            if brick.special_effect is not None:
                ...
                self.effect_start_time = dt.datetime.now()
    ...
    def update(self):
        ...
        if self.reset_effect:
            if dt.datetime.now() - self.effect_start_time >= dt.timedelta(seconds=config.effect_duration):
                self.reset_effect(self)
                self.reset_effect = None
